...
def dataset_summary(df: pd.DataFrame, target_etfs: List[str]) -> dict:
    ...
    # Columns are just ETF names (e.g., "GDX", "XLK"), not "GDX_close"
    etf_cols = [etf for etf in target_etfs if etf in df.columns]
...
def build_features(df_raw: pd.DataFrame, 
    ...
    # Columns are just ETF names (e.g., "GDX"), not "GDX_close"
    features_list = []
    feature_names = []
    
    for ticker in feature_tickers:
        # Column is just the ticker name, not ticker_close
    ...
    if not features_list:
        raise ValueError("No features could be built - check column names")
    ...
    for etf in target_etfs:
        ...
        next_ret = df[etf].pct_change(1).shift(-1)
